...
from tests.customer_management_api.db.account_holder import get_account_holder, get_active_account_holder
...
from tests.customer_management_api.payloads.getbycrdentials import (
    ...
    malformed_request_body, missing_credentials_request_body, wrong_validation_request_body,
)
from tests.customer_management_api.requests.getbycredentials import send_post_getbycredentials, \
    send_malformed_post_getbycredentials, send_invalid_post_getbycredentials
...
def setup_account_holder(retailer_slug: str, request_context: dict):
...
def setup_check_enrolment_response_status_code(status_code: int, request_context: dict):
...
def check_getbycredentials_response(request_context: dict):
...
def post_getbycredentials_invalid_retailer(request_context: dict):
...
def post_invalid_token_request(retailer_slug: str, request_context: dict):
    request_context["retailer_slug"] = retailer_slug
    request_body = all_required_credentials()
...
def check_account_holder_is_active(request_context: dict):
    request_body = json.loads(request_context["response"].request.body)
    email = request_body["credentials"]["email"]
    retailer_slug = request_context["retailer_slug"]
...
def check_getbycredentials_response_status_code(status_code: int, request_context: dict):
...
def non_existent_account_holder(retailer_slug: str, request_context: dict):
    request_context["retailer_slug"] = retailer_slug
