Feature: Bink BPL - Jigshaw egift - End campaign and delete pending rewards
  ...
  Background:
    ...
    And the retailer has a old-campaign reward config configured with validity_days: 30, and a status of ACTIVE and a JIGSAW_EGIFT fetch type
    And the retailer has a new-campaign reward config configured with validity_days: 30, and a status of ACTIVE and a JIGSAW_EGIFT fetch type
    ...
    And the account holders each have 2 pending rewards for the old-campaign campaign with a value of 10000
    And the account holders each have 2 pending rewards for the new-campaign campaign with a value of 10000
  ...
  Scenario: End campaign - delete the pending rewards
    When the retailer's old-campaign campaign status is changed to ended with pending rewards to be deleted
    ...
    Then there are 2 rewards for the old-campaign reward config, with allocated set to false and deleted set to true
    ...
    And 2 pending rewards are available to each account holder
    And 6 issued rewards are available to each account holder
