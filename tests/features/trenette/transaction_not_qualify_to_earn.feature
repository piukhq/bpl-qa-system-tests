...
Feature: Bink BPL - Transaction doesn't meet threshold
  ...
  Scenario Outline: Transaction doesn’t qualify for earn
    ...
    And the <campaign_type> campaign has an <loyalty_type> earn rule with a threshold of 500, an increment of <increment>, a multiplier of 1 and max amount of <max_amount>
  ...
  Scenario Outline: Account holder already has balance but new transaction isn't qualify for earn and balance
    ...
    Examples:
      | campaign_type          | transaction_amount_1 | transaction_amount_2 | expected_balance | expected_reward | loyalty_type | increment | reward_rule | reward_slug     | max_amount |
      | trenette-stmp-campaign | 770                  | 480                  | 100              | 0               | STAMPS       | 100       | 700         | free-item       | None       |
