...
Feature: Bink BPL - Ensure a customer can enrol successfully and activation started
  ...
  Scenario: Enrolment Trigger - No active campaign (draft and then activate)
    ...
    And the retailer has a WELCOME_EMAIL email template configured with template id 4033322 and first_name, last_name, account_number, marketing_token variables
